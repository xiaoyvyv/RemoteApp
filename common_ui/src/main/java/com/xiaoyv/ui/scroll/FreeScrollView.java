...
public class FreeScrollView extends FrameLayout {
    ...
    public boolean fullScroll(int direction, boolean horizontal) {
        if (!horizontal) {
            ...
            int t = mTempRect.top;
            int b = mTempRect.bottom;
            return scrollAndFocus(0, 0, 0, direction, t, b);
        }
    }
    ...
    public void fling(int velocityX, int velocityY) {
        if (getChildCount() > 0) {
            ...
            final boolean movingTop = velocityX > 0;

            View newFocused = findFocusableViewInMyBounds(movingTop, mScroller.getFinalX(), movingDown, mScroller.getFinalY(), findFocus());
            if (newFocused == null) {
                newFocused = this;
            }
