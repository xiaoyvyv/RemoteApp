...
public class ChannelManager implements MessageHandler {
    ...
    private static final long DEFAULT_WAIT_TIMEOUT = Long.valueOf(System.getProperty(PROPERTY_TIMEOUT, "120000"));

    private final HashMap x11_magic_cookies = new HashMap();

    /*package*/ TransportManager tm;

    private final Vector channels = new Vector();
    private int nextLocalChannel = 100;
    private boolean shutdown = false;
    private int globalSuccessCounter = 0;
    private int globalFailedCounter = 0;

    private final HashMap remoteForwardings = new HashMap();

    private final Vector listenerThreads = new Vector();
    ...
    public void requestGlobalTrileadPing() throws IOException {
        ...
        try {
            ...
            throw (IOException) new IOException("The ping request failed.", e);
        }
    }
    ...
    public void requestPTY(Channel c, String term, int term_width_characters, int term_height_characters,
        ...
        try {
            ...
            throw (IOException) new IOException("PTY request failed", e);
        }
    }
    ...
    public void requestX11(Channel c, boolean singleConnection, String x11AuthenticationProtocol,
        ...
        try {
            ...
            throw (IOException) new IOException("The X11 request failed.", e);
        }
    }
    ...
    public void requestSubSystem(Channel c, String subSystemName) throws IOException {
        ...
        try {
            ...
            throw (IOException) new IOException("The subsystem request failed.", e);
        }
    }
    ...
    public void requestExecCommand(Channel c, String cmd) throws IOException {
        ...
        try {
            ...
            throw (IOException) new IOException("The execute request failed.", e);
        }
    }
    ...
    public void requestShell(Channel c) throws IOException {
        ...
        try {
            ...
            throw (IOException) new IOException("The shell request failed.", e);
        }
    }
        ...
        synchronized (channels) {
            ...
            for (int i = 0; i < channels.size(); i++) {
                ...
                synchronized (c) {
                    c.eof();
                    c.state = Channel.STATE_CLOSED;
                    c.setReasonClosed(new IOException("The connection is being shutdown", cause));
                    c.closeMessageRecv = true; /*
    ...
    private IOException ioException(String msg, Channel c) {
        return (IOException) new IOException(msg, c.getReasonClosedCause());
    }
}
